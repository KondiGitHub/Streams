...
public class ProcessingEmployees {
	...
	public static void main(String[] args) {
		...
		System.out.println("==============================================");
		list.stream()
		.filter(emp->emp.getFirstName().compareToIgnoreCase("Jason")!=0)
		.filter(emp->emp.getFirstName().length()>5)
		.forEach(System.out::println);
		
		System.out.println("==============================================");
		
		Stream<Employee> fourToSixThousand =  list.stream().filter(e -> (e.getSalary() >= 4000 && e.getSalary() <= 6000));
		...
		System.out.printf("%nFirst employee who earns $4000-$6000:%n%s%n", fourToSixThousand.findFirst().get());
...
System.out.printf("%nEmployees in descending order by last name then first:%n");
     ...
     // count number of Employees in each department
        ...
        Map<String, Long> employessCountByDepartment = list.stream().collect(Collectors.groupingBy(Employee::getDepartment,Collectors.counting()));
        
        employessCountByDepartment.forEach((department,employeeCount)->System.out.printf("%s has %d employees%n", department,employeeCount));
